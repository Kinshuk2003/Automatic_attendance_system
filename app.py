...
def train():
    if 'username' in session:
        ...
        trained = trainmodel(username,classroom_name)

        if trained:
            cur = mysql.connection.cursor()
            cur.execute("UPDATE classroom SET model = 1 WHERE username = %s AND classroom = %s", (username, classroom_name))
            cur.close()
            mysql.connection.commit()
            return redirect('classroom?classroom=' + classroom_name)
...
def prediction():
    if 'username' in session:
        if request.method == 'POST':
            ...
            image = cv.imread('temp/temp.jpg')
            true = predict(image,username,classroom_name)
            if true:
        ...
        return render_temmple('login.html')
